...
func Migrate(ctx context.Context, logger *slog.Logger, cfg *config.Config) error {
	...
	if from == int32(len(m.Migrations)) {
		logger.Info(fmt.Sprintf("database schema up to date, version %d", len(m.Migrations)))

	} else {
		logger.Info(fmt.Sprintf("migrated database schema, from %d to %d", from, len(m.Migrations)))
	}
	return nil
}
